...
export class ProductService {
    constructor(
      ...
      async createProduct(createProductDto: CreateProductDto, file: Express.Multer.File) {
        const product = await this.findProductBySku(
            createProductDto.sku
        ).catch(() => undefined);
        let secure_url: null | string = null;
        if (product) {
          throw new BadRequestException('product with this SKU registered in system.');
        }
      ...
      async getAllProducts(): Promise<ProductEntity[]> {
        return await this.productRepository.find({relations: ['category'], order: {
          createdAt: 'DESC'
        }});
      }
}
